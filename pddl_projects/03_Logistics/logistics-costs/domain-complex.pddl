...
(define (domain logistics)
	...
	(:action move-truck
		...
		:precondition (and (vehicle-at ?t ?from) (road ?from ?to))
		:effect (and (not (vehicle-at ?t ?from)) (vehicle-at ?t ?to) (increase (total-cost) (* (distance ?from ?to) (travel-cost ?t))))
	)
	
	(:action move-plane
		:parameters (?a - plane ?from ?to - location)
		:precondition (and (vehicle-at ?a ?from) (corridor ?from ?to))
		:effect (and (not (vehicle-at ?a ?from)) (vehicle-at ?a ?to) (increase (total-cost) (* (distance ?from ?to) (travel-cost ?a))))
	)
	...
	(:action load
		...
		:precondition (and (vehicle-at ?v ?where) (package-at ?p ?where))
		:effect (and (not (package-at ?p ?where)) (package-in ?p ?v))
	)
	
	(:action unload
		:parameters (?v - vehicle ?p - package ?where - location)
		:precondition (and (vehicle-at ?v ?where) (package-in ?p ?v))
		:effect (and (not (package-in ?p ?v)) (package-at ?p ?where))
	)
)